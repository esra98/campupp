...
export default function CampsiteAdd(){
    ...
    useEffect(()=>{
        setTitle(event?.title)
            ...
            setRequirement(event?.requirement)
            ...
            setContactInstagram(event?.contactInstagram)
            setContactFacebook(event?.contactFacebook)
    ...
    async function editEvent(ev) {
        ...
        if(contactPhone){
            const checkPhone = /^(((\+|00)?(90)|0)[-| ]?)?((5\d{2})[-| ]?(\d{3})[-| ]?(\d{2})[-| ]?(\d{2}))$/.test(contactPhone)
            if(!checkPhone){
        ...
        if(contactEmail){
            const checkMail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(contactEmail)
            if(!contactEmail){
        ...
        const data = {eventId: event._id,title, description,price,startDate,endDate,category,images,requirement, contactName,contactPhone,contactEmail,contactInstagram,contactFacebook}
        const res = await axios.put('/api/events',{...data,editInfo:true});
        if(res.data=="ok"){
            toast.success('Etkinliğiniz düzenlendi')
        }else{
        ...
        <Layout>
            ...
            {event && (
                <div className=' w-full'>
                    <form onSubmit={editEvent} className="p-5 m-5 border rounded-2xl bg-gray-100">
                            ...
                            <div className='flex gap-6'>
                                <div>
                                    <p className="mt-1 text-sm leading-6 text-gray-600">
                                        Başlangıç Tarihi: {startDate?.substring(0, 10)}
                                    </p>
                                    <div className="mt-2">
                                        <input type="date"
                                        value={startDate}
                                ...
                                <div>
                                    <p className="mt-1 text-sm leading-6 text-gray-600">
                                        Bitiş Tarihi: {endDate?.substring(0, 10)}
                                    </p>
                                    <div className="mt-2">
                                        <input type="date"
                                        value={endDate}
