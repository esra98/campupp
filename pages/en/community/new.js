...
export default function CampsiteAdd(){
    ...
    const [category,setCategory] = useState("Search for camp mates");
    const router = useRouter();
    async function saveBoard(ev) {
        ev.preventDefault();
        if(title==""){
            toast.error('Title field cannot be left blank')
            return
        }
        if(shortDesc==""){
            toast.error('Description field cannot be left blank')
            return
        }
        ...
        if(res.data=="ok"){
            ...
            toast.error('The operation could not be completed, please try again later.')
        }
    }
    ...
    async function uploadImages(ev){
        if(title==""){
          toast.error('Before you start adding photos, fill in the title field.')
          return
        }
    ...
    function removePhoto(ev, filename){
        if(title==""){
          toast.error('Press the "Save" button before you start deleting the photo.')
          return
        }
        ev.preventDefault();
            ...
            <div className=' w-full px-16'>
                    <form onSubmit={saveBoard} className="p-5 m-5 border rounded-2xl bg-gray-100">
                                  ...
                                  <div className="relative flex gap-x-3">
                                    ...
                                    <div className="text-sm leading-6">
                                      <label htmlFor="comments" className="font-medium text-gray-900">
                                        If you want to create the board anonymously, select the box next to it.
                                      </label>
                                    </div> 
                                  </div>
                                </div>
                        ...
                        <div className="col-span-full mt-5">
                            ...
                            <p className="mt-3 text-sm leading-6 text-gray-600">Write down the details about the topic you want to share. </p>
                            <div className="mt-2">
                                <textarea
                                rows={3}
                        ...
                        <div>
                            ...
                            <div className="mt-2">
                                ...
                                {!images?.length &&(
                                    <div>You have not uploaded images to your message board.</div>
                                )}
                                </div>
                            </div>
                        </div>   
                        <div className="sm:col-span-3">
                            ...
                            <div className="mt-2">
                                ...
                                >
                                <option>Search for camp mates</option>
                                <option>Get ideas about the campground</option>
                                <option>Get ideas about camping gear</option>
                                <option>Camping equipment swap and barter</option>
                                </select>
                            </div>
                        </div>
                        <button
