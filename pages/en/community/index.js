...
const categories = [
  ...
  'Get ideas about camping gear',
  'Camping equipment swap and barter'
]
...
export default function Community() {
  ...
  return (
    <Layout>
      <Head>
          <title>Ask to Campers - Camping Community</title>
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="relative p-6 flex-auto">
                              <p>Are you sure you want to delete your message board?</p>
                            </div>
                            {/*header*/}
                            <div className="
                            flex 
                ...
                <div>
                  ...
                  <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="md:flex items-baseline justify-between border-b border-gray-200 pb-6">
                      <div>
                        <h1 className="text-3xl mt-5 font-semibold tracking-tight text-green-800">Message Boards</h1>
                        <p className="mt-2 font-light text-gray-500 sm:text-xl dark:text-gray-400">Create a message board on topics you have in mind and get ideas from the camper community.</p>        
                      </div>                  
                      <div className="flex items-center mt-5">
                        <div>
                            <Link href="/en/community/new" className="p-2 gap-2 px-4 border flex rounded-xl  hover:bg-transparent shadow hover:shadow-md">
                            ...
                            <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
                              {boards?.map((board) => (
                                <div key={board._id} href={'/community/'+ board._id} className="group overflow-hidden rounded-xl shadow-md pt-3">
                                  <span className="bg-green-100 text-green-800 text-sm font-medium shadow mr-2 px-2.5 py-1 rounded ml-2">{board.category}</span>
                                  <div className="max-w-sm p-6 ">
                                  <div className="italic text-xs text-gray-700">{board.anonymous ? "anonim" : board.user}:</div>
                                      <a href="#">
                                          <h5 className="mb-2 text-2xl font-semibold tracking-tight text-gray-900">{board.title}</h5>
                                      </a>
                                      <p className="mb-3 font-normal text-gray-700">{board.detail}</p>
                                      <a href={"/community/" + board._id} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300">
                                          See More
                                      ...
                                      {board.replies?.length>0 && (
                                        <p className="mt-2 text-xs text-gray-700">{board.replies?.length} have joined the board</p>
                                      )}
                                  </div>
                                  ...
                                  <div className='grid justify-items-end w-full'>
                                    <div className='flex gap-2 w-full'>
                                      <button onClick={()=>{setIsDeleteModalOpen(true);setDeletedBoardId(board._id);setDeletedImages(board.images)}}  className="p-2 w-full text-red-700 text-center gap-2 px-4 border flex hover:bg-transparent shadow hover:shadow-md">
                                        ...
                                        <div className="font-semibold">Delete your board</div>
                                      </button>
                                    </div>
                                  </div>    
                                  )}
                              ...
                              {boards?.length==0 && (
                                <div className="pt-32 flex items-center mt-5">
                                  <div className="text-center w-full">
                                      ...
                                      />
                                      <p className="mb-3 font-semibold">No Message Board Found Based on Your Search Criteria </p>
